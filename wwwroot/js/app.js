        ...
        return new Promise(function(resolve) {
            try {
                ...
                request.onupgradeneeded = function(event) {
                    ...
                    ['bookmarks', 'notes', 'highlights', 'history', 'progress', 'planProgress', 'books'].forEach(function(store) {
...
window.bibleImage = {
    ...
    _titleFonts: [
        ...
        "'Cambria', 'Hoefler Text', Utopia, serif",
        "'Garamond', 'Baskerville', 'Baskerville Old Face', serif",
    ...
    _pickTitleFont: function () {
        var idx = Math.floor(Math.random() * this._titleFonts.length);
    ...
    share: function (texte, reference, url) {
        ...
        // 1. Try text-only share first (works on all mobile browsers)
        if (navigator.share) {
            navigator.share({ title: reference + ' — LSG', text: shareText })
                .catch(function () {});
            return;
        }

        // 2. Fallback: generate image + share or download
    ...
    shareVersetDuJour: function (texte, reference, url) {
        ...
        // 1. Try text-only share first (works on all mobile browsers)
        if (navigator.share) {
            navigator.share({ title: 'Verset Du Jour — LSG', text: shareText })
                .catch(function () {});
            return;
        }
    ...
    promptInstall: function () {
        ...
        try {
            ...
            this.deferredPrompt.userChoice.then(function (result) {
                ...
                console.error('[PWA] Install prompt userChoice failed:', err);
                window.bibleInstall.deferredPrompt = null;
                window.bibleInstall.canInstall = false;
            });
        } catch (err) {

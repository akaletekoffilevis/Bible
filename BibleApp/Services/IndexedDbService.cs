...
public class IndexedDbService
{
    ...
    {
        try { return await _js.InvokeAsync<T?>("bibleDb.getById", storeName, id); }
        catch (Exception ex) { Console.Error.WriteLine($"[IndexedDb] Échec GetAsync {storeName}/{id}: {ex.Message}"); return null; }
    }
    ...
    public async Task<List<Models.Highlight>> GetHighlightsAsync()
    {
        ...
        catch (Exception ex) { Console.Error.WriteLine($"[IndexedDb] Échec GetHighlights: {ex.Message}"); return new List<Models.Highlight>(); }
    }
    ...
    public async Task<Models.Highlight?> GetHighlightAsync(string verseId)
    {
        ...
        catch (Exception ex) { Console.Error.WriteLine($"[IndexedDb] Échec GetHighlight {verseId}: {ex.Message}"); return null; }
    }
